...
function main(distribute , parts)
    ...
    # Define problem specific values (all in SI units)
    H = 1.1 # Height of the domain
    Lf = pi*10 # Length of the entire domain
    Ld = pi*5 # Length at which damping starts
    Ld1 = pi*10 - 3/7 # Length after which there is 100% damping
    ...
    add_tag_from_tags!(labels,"BCLeft",[3])
    add_tag_from_tags!(labels,"BCRight",[4])
    ...
    Σbottom = BoundaryTriangulation(model,tags=["Bottom"])
    dΣbottom = Measure(Σbottom,degree)
    ...
    # Initial Conditions and forcing function 
    pres_base(t) = x-> ρf*g*(H - x[2])
    pres_field(t) = x-> ρf*g*(H  - x[2]) + ρf*g*η₀*sin(ω*t)
    u_field(t) = x-> VectorValue(0,0)
    u_slip(t) = x -> VectorValue(0.0,0.0)
    disp_field(t) = x -> 0.0
    disp_force(t) = x-> η₀*sin(ω*t)
   
    # Define trial FESpaces
    U = TransientTrialFESpace(V,[u_slip])
    P = TransientTrialFESpace(Q)
    D = TransientTrialFESpace(S,[disp_force])
    Xfluid = TransientMultiFieldFESpace([U,P])
    X = TransientMultiFieldFESpace([D,U,P]; style = mfs)

    # Initial condition functions 
    pressure_initial(t) = pres_base(0.0) # Initial Pressure
    velocity_initial(t) = u_field(0.0) # Initial Velocity
    sol_dis_initial(t) = disp_field(0.0)

    # Initial condition (velocity and acceleration terms) (time integration)
    pressure_VelAcc = x-> 0.0 # Initial vel/acc terms for pressure
    velocity_VelAcc = x-> VectorValue(0.0,0.0) # Initial vel/acc terms for velocity
    sol_dis_VelAcc = x-> 0.0 # Initial vel/acc terms for solid displacement
    ...
    c1 = 0.10
    c2 = 10
    function frac(x,Ld,Lf,Ld1)
        if(x[1] ≈ Ld1 || x[1] > Ld1)
            return  1.0
        elseif(x[1] >= Ld)
            return (x[1] - Ld)/(Ld1 - Ld) * (H - x[2])/(H)
        else
            return 0.0
        end
    end
    α(x) = (1.0 - c1*frac(x,Ld,Lf,Ld1)*frac(x,Ld,Lf,Ld1))*(1.0 - (1.0 - exp(c2*frac(x,Ld,Lf,Ld1)*frac(x,Ld,Lf,Ld1)))/(1.0 - exp(c2)))
    αvec(x) = VectorValue(α(x),α(x))
    damping = interpolate_everywhere([α, αvec, α], X(t0))
    dampArr = get_free_dof_values(damping)

    # Weak from definition (uncoupled system)
    ...
    params = Dict("VΓ" =>VΓ, "SΓ" => SΓ, "dommap_V" =>dommap_V, "dommap_S" =>dommap_S, "αf" =>αf, "αs" =>αs, "dampArr" => dampArr)
    ...
    x_initial = interpolate_everywhere([sol_dis_initial(t0), velocity_initial(t0), pressure_initial(t0)], X(t0))
    v_initial = interpolate_everywhere([sol_dis_VelAcc(t0), velocity_VelAcc(t0), pressure_VelAcc(t0)], X(t0))
    a_initial = interpolate_everywhere([sol_dis_VelAcc(t0), velocity_VelAcc(t0), pressure_VelAcc(t0)], X(t0))
    ...
    function plot_residual(solver)
        ...
        for (x, y) in zip(iter, res)
            annotate!(x , y, text(" "*string(round(y, sigdigits=3)), :left, 8, :blue))
        end
