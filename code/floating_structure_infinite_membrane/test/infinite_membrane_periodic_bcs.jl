...
function main(distribute , parts)
    ...
    T = ((ω*ω)/(kλ*kλ))*((ρf)/(kλ*tanh(kλ*H)) + ρs*hs) # pretension on the membrane
    ...
    V = TestFESpace(Ωf,reffeᵤ,dirichlet_tags = ["Bottom","Left","Right"], # ,"Left","Right"
                        dirichlet_masks =[(false,true),(true,true),(true,true)]) # ,(true,false),(true,false)
    ...
    uₓ(t,x) = -ω*η₀*(cosh(kλ*(H - x[2]))/(sinh(kλ*H)))*cos(kλ*x[1] - ω*t + ϕ)
    uy(t,x) = -ω*η₀*(sinh(kλ*(H - x[2]))/(sinh(kλ*H)))*sin(kλ*x[1] - ω*t + ϕ)
    u_field(t) = x-> VectorValue(uₓ(t,x),uy(t,x))
    pres_field(t) = x-> ρf*g*(H - x[2]) - ρf*((η₀*ω*ω)/(kλ))*(cosh(kλ*(H - x[2]))/(sinh(kλ*H)))*cos(kλ*x[1] - ω*t + ϕ)
    disp_field(t) = x-> η₀*cos(kλ*x[1] - ω*t + ϕ)
    u_slip(t) = x -> VectorValue(0.0,0.0)
    pres_base(t) = x-> ρf*g*(H - x[2])
   
    # Define trial FESpaces
    U = TransientTrialFESpace(V,[u_slip,u_field,u_field])
    P = TransientTrialFESpace(Q)
    D = TransientTrialFESpace(S,[disp_field])
    Xfluid = TransientMultiFieldFESpace([U,P])
    X = TransientMultiFieldFESpace([D,U,P]; style = mfs)

    # Initial condition functions 
    pressure_initial(t) = pres_field(0.0) # Initial Pressure
